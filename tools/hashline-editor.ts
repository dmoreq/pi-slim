...
import { normalizeToLF, restoreLineEndings, stripBom, detectLineEnding } from "../hashline/normalize.js";
...
async function makeEdit(
  ...
  const resolvedEdits = edits.map((e) => {
    ...
    if ("append" in lo && typeof lo.append === "string") return { op: "append_at" as const, pos: parseTag(lo.append), lines };
    if ("prepend" in lo && typeof lo.prepend === "string") return { op: "prepend_at" as const, pos: parseTag(lo.prepend), lines };
