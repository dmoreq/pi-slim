/**
 * Text normalization utilities — extracted from oh-my-pi.
 *
...
// ═══════════════════════════════════════════════════════════════════════════
// Inline padding helper (replaces @oh-my-pi/pi-tui import)
// ═══════════════════════════════════════════════════════════════════════════
...
function buildIndentProfile(text: string): IndentProfile {
  ...
  for (const line of lines) {
    ...
    if (indent.length > 0) {
      ...
      else if (char !== currentChar) mixed = true;
    }
  }
  ...
  return { lines, indentCounts, char, spaceOnly, tabOnly, mixed, unit, nonEmptyCount };
}
...
export function convertLeadingTabsToSpaces(text: string, spacesPerTab: number): string {
  ...
  return text
    ...
    .map(line => {
      ...
      return padding(leading.length * spacesPerTab) + trimmed;
    })
    .join("\n");
}
...
function isIndentationOnlyRewrite(oldText: string, newText: string): boolean {
  ...
  return oldLines.every((line, i) => line.trim() === newLines[i].trim());
}
...
function maybeConvertTabIndentation(
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    if (actualIndent.length !== oldIndent.length * actualProfile.unit) return undefined;
  }
...
function computeUniformIndentDelta(oldProfile: IndentProfile, actualProfile: IndentProfile): number | undefined {
  ...
  return deltas.every(v => v === delta) ? delta : undefined;
}

function applyIndentDelta(text: string, delta: number, indentChar: string): string {
  return text.split("\n").map(line => {
    if (!isNonEmptyLine(line)) return line;
...
function hasNonEmptyIndentProfiles(...profiles: IndentProfile[]): boolean {
  return profiles.every(p => p.nonEmptyCount > 0);
}

function hasMixedIndentation(...profiles: IndentProfile[]): boolean {
  return profiles.some(p => p.mixed);
}
...
export function adjustIndentation(oldText: string, actualText: string, newText: string): string {
  ...
  if (!hasNonEmptyIndentProfiles(oldProfile, actualProfile, newProfile)) return newText;
  if (hasMixedIndentation(oldProfile, actualProfile, newProfile)) return newText;
