...
function stripLeadingHashlinePrefixes(line: string): string {
  let result = line;
  let prev: string;
  do { prev = result; result = result.replace(HASHLINE_PREFIX_RE, ""); } while (result !== prev);
  return result;
}
...
export class HashlineMismatchError extends Error {
  ...
  constructor(
    ...
    for (const m of mismatches) remaps.set(`${m.line}${m.expected}`, `${m.line}${computeLineHash(m.line, fileLines[m.line - 1])}`);
    this.remaps = remaps;
  }
...
function buildDisplayLineSet(mismatches: HashMismatch[], fileLen: number): Set<number> {
  ...
  for (const m of mismatches) {
    for (let i = Math.max(1, m.line - MISMATCH_CONTEXT); i <= Math.min(fileLen, m.line + MISMATCH_CONTEXT); i++) s.add(i);
  }
  return s;
}
...
function dedupeEdits(edits: HashlineEdit[]): void {
  ...
  for (let i = 0; i < edits.length; i++) {
    ...
    if (seen.has(key)) drop.add(i); else seen.set(key, i);
  }
  for (let i = edits.length - 1; i >= 0; i--) { if (drop.has(i)) edits.splice(i, 1); }
}

function sortKey(edit: HashlineEdit, lineCount: number): { sortLine: number; precedence: number } {
  switch (edit.op) {
    case "replace_line": return { sortLine: edit.pos.line, precedence: 0 };
    case "replace_range": return { sortLine: edit.end.line, precedence: 0 };
    case "append_at": return { sortLine: edit.pos.line, precedence: 1 };
    case "prepend_at": return { sortLine: edit.pos.line, precedence: 2 };
    case "append_file": return { sortLine: lineCount + 1, precedence: 1 };
    case "prepend_file": return { sortLine: 0, precedence: 2 };
  }
}
...
function applyEdit(
  ...
  if (op === "replace_line") {
    ...
    } else { fileLines.splice(edit.pos.line - 1, 1, ...edit.lines); track(edit.pos.line); }
    ...
    if (count === edit.lines.length && orig.every((l, i) => l === edit.lines[i])) {
      noopEdits.push({ editIndex, loc: `${edit.pos.line}${edit.pos.hash}-${edit.end.line}${edit.end.hash}`, current: orig.join("\n") });
    } else { fileLines.splice(edit.pos.line - 1, count, ...edit.lines); track(edit.pos.line); }
  } else if (op === "append_at") {
    if (edit.lines.length === 0) { noopEdits.push({ editIndex, loc: `${edit.pos.line}${edit.pos.hash}`, current: originalFileLines[edit.pos.line - 1] }); }
    else { fileLines.splice(edit.pos.line, 0, ...edit.lines); track(edit.pos.line + 1); }
  } else if (op === "prepend_at") {
    if (edit.lines.length === 0) { noopEdits.push({ editIndex, loc: `${edit.pos.line}${edit.pos.hash}`, current: originalFileLines[edit.pos.line - 1] }); }
    else { fileLines.splice(edit.pos.line - 1, 0, ...edit.lines); track(edit.pos.line); }
  } else if (op === "append_file") {
    if (edit.lines.length === 0) { noopEdits.push({ editIndex, loc: "EOF", current: "" }); }
    else {
      if (fileLines.length === 1 && fileLines[0] === "") { fileLines.splice(0, 1, ...edit.lines); track(1); }
      else { fileLines.splice(fileLines.length, 0, ...edit.lines); track(fileLines.length - edit.lines.length + 1); }
    }
...
function validateRefs(edits: HashlineEdit[], fileLines: string[], warnings: string[]): HashMismatch[] {
  ...
  const check = (ref: { line: number; hash: string }) => {
    if (ref.line < 1 || ref.line > fileLines.length) throw new Error(`Line ${ref.line} does not exist (file has ${fileLines.length} lines)`);
    const actual = computeLineHash(ref.line, fileLines[ref.line - 1]);
    if (actual === ref.hash) return;
    const rebased = tryRebaseAnchor(ref, fileLines);
    if (rebased !== null) {
      warnings.push(`Auto-rebased anchor ${ref.line}${ref.hash} \u2192 ${rebased}${ref.hash} (line shifted within \u00b1${ANCHOR_REBASE_WINDOW}; hash matched).`);
      ref.line = rebased;
    } else {
  ...
  for (const edit of edits) {
    switch (edit.op) {
      case "replace_line": check(edit.pos); break;
      case "replace_range": check(edit.pos); check(edit.end); if (edit.pos.line > edit.end.line) throw new Error(`Range start ${edit.pos.line} > end ${edit.end.line}`); break;
      case "append_at": case "prepend_at": check(edit.pos); ensureContent(edit); break;
      case "append_file": case "prepend_file": ensureContent(edit); break;
    }
  }
  return mismatches;
}
...
function buildResult(fileLines: string[], fc: number | undefined, warnings: string[], noopEdits: Array<{ editIndex: number; loc: string; current: string }>) {
  return { lines: fileLines.join("\n"), firstChangedLine: fc, ...(warnings.length ? { warnings } : {}), ...(noopEdits.length ? { noopEdits } : {}) };
}
...
export function applyHashlineEdits(text: string, edits: HashlineEdit[]): {
  ...
  const sorted = edits.map((e, i) => ({ edit: e, idx: i, ...sortKey(e, fileLines.length) }))
    .sort((a, b) => b.sortLine - a.sortLine || a.precedence - b.precedence || a.idx - b.idx);

  for (const { edit, idx } of sorted) applyEdit(edit, fileLines, originalFileLines, idx, noopEdits, l => { if (fc === undefined || l < fc) fc = l; });
