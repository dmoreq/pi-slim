...
import { HASHLINE_CONTENT_SEPARATOR, computeLineHash } from './line-hash.js'
...
function parseEntries(lines: string[]): Entry[] {
  ...
  for (const line of lines) {
    ...
    if (!p) {
      entries.push({ kind: 'meta', oldLine: 0, newLine: 0, content: '', raw: line })
      continue
    }

    const isEl = p.content === ELLIPSIS
    if (p.kind === '+') {
      if (!neu) neu = p.lineNumber
      else neu += p.lineNumber - (entries[entries.length - 1]?.newLine ?? neu)
      entries.push({ kind: '+', oldLine: old || p.lineNumber, newLine: neu, content: p.content })
      if (!isEl) neu++
    } else if (p.kind === '-') {
      if (!old) old = p.lineNumber
      else old += p.lineNumber - (entries[entries.length - 1]?.oldLine ?? old)
...
function pairMods(runs: Array<{ kind: EntryKind; entries: Entry[] }>): Array<{ kind: EntryKind; entries: Entry[] }> {
  ...
  for (let i = 0; i < runs.length; i++) {
    ...
    const dels = run.entries.filter(e => e.kind !== 'meta' && e.content !== ELLIPSIS) as Array<
      Entry & { kind: '-' | '+' | ' ' }
    >
    const adds = next.entries.filter(e => e.kind !== 'meta' && e.content !== ELLIPSIS) as Array<
      Entry & { kind: '-' | '+' | ' ' }
    >
    ...
    for (let p = 0; p < pairCount; p++)
      mods.push({ kind: '*', oldLine: dels[p].oldLine, newLine: adds[p].newLine, content: adds[p].content })
...
function fmt(e: Entry): string {
  ...
  switch (e.kind) {
    case '+':
      return `+${e.newLine}${computeLineHash(e.newLine, e.content)}${HASHLINE_CONTENT_SEPARATOR}${e.content}`
    case '-':
      return `-${e.oldLine}${PLACEHOLDER}${HASHLINE_CONTENT_SEPARATOR}${e.content}`
    case ' ':
      return ` ${e.newLine}${computeLineHash(e.newLine, e.content)}${HASHLINE_CONTENT_SEPARATOR}${e.content}`
    case '*':
      return `*${e.newLine}${computeLineHash(e.newLine, e.content)}${HASHLINE_CONTENT_SEPARATOR}${e.content}`
...
function collapseMiddle(entries: Entry[], maxRun: number): string[] {
  ...
  return [
    ...entries.slice(0, maxRun).map(fmt),
    ` ... ${hidden} more unchanged lines`,
    ...entries.slice(-maxRun).map(fmt),
  ]
}
...
export function buildCompactHashlineDiffPreview(
  ...
  for (let i = 0; i < runs.length; i++) {
    ...
    switch (run.kind) {
      case 'meta':
        run.entries.forEach(e => out.push(fmt(e)))
        break
      case '+':
        run.entries.forEach(e => {
          if (e.content !== ELLIPSIS) added++
          out.push(fmt(e))
        })
        break
      case '-':
        run.entries.forEach(e => {
          if (e.content !== ELLIPSIS) removed++
          out.push(fmt(e))
        })
        break
      case '*':
      ...
      case ' ':
        if (i === 0) {
          out.push(...run.entries.slice(-maxRun).map(fmt))
          break
        }
        if (i === runs.length - 1) {
          out.push(...run.entries.slice(0, maxRun).map(fmt))
          break
        }
