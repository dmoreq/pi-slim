...
function createChunkEmitter(options: ResolvedOptions, formatLine = formatHashLine): ChunkEmitter {
  ...
  const pushLine = (line: string): string[] => {
    ...
    if (outLines.length > 0 && (outLines.length >= options.maxChunkLines || outBytes + sepBytes + lineBytes > options.maxChunkBytes)) {
      const f = flush(); if (f) chunks.push(f);
    }
...
export async function* streamHashLinesFromLines(
  ...
  if (!sawAny) { for (const out of emitter.pushLine("")) yield out; }
