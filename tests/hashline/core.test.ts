...
describe('applyHashlineEdits', () => {
  ...
  it('replaces a line', () => {
    const r = applyHashlineEdits(text, [
      { op: 'replace_line', pos: { line: 1, hash: ch(1, 'line one') }, lines: ['replaced one'] },
    ])
  ...
  it('replaces a range', () => {
    const r = applyHashlineEdits(text, [
      {
        op: 'replace_range',
        pos: { line: 1, hash: ch(1, 'line one') },
        end: { line: 2, hash: ch(2, 'line two') },
  ...
  it('appends at a line', () => {
    const r = applyHashlineEdits(text, [
      { op: 'append_at', pos: { line: 2, hash: ch(2, 'line two') }, lines: ['appended'] },
    ])
  ...
  it('prepends at a line', () => {
    const r = applyHashlineEdits(text, [
      { op: 'prepend_at', pos: { line: 2, hash: ch(2, 'line two') }, lines: ['prepended'] },
    ])
    expect(r.lines).toBe('line one\nprepended\nline two\nline three')
  })
  ...
  it('throws HashlineMismatchError on bad hash', () => {
    expect(() =>
      applyHashlineEdits(text, [{ op: 'replace_line', pos: { line: 1, hash: 'xx' }, lines: ['x'] }])
    ).toThrow(HashlineMismatchError)
  })

  it('detects noop edits', () => {
    const r = applyHashlineEdits(text, [
      { op: 'replace_line', pos: { line: 1, hash: ch(1, 'line one') }, lines: ['line one'] },
    ])
