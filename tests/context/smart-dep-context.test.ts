...
describe('SmartDependencyContextGenerator', () => {
  ...
  it('prioritizes god nodes in dependency context', () => {
    ...
    expect(context).not.toContain('WORKFLOW OPTIMIZATION')
  })

  it('includes community context when relevant', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: true,
  ...
  it('resolves community linkage case-insensitively', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: false,
        targetSymbols: [],
  ...
  it('surfaces top god nodes even without specific symbol mentions', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: false,
        targetSymbols: [],
  ...
  it('resolves architecture symbols against community nodes case-insensitively', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: false,
        targetSymbols: [],
  ...
  it('handles null graph analysis gracefully', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: true,
        targetSymbols: ['authenticate'],
  ...
  it('includes suboptimal pattern tool suggestions only', () => {
    const insights: ContextInsights = {
      editingIntent: {
        detected: false,
        targetSymbols: [],
    ...
    expect(context).toContain('🔧 TOOL PATTERNS')
    expect(context).toContain('Use hashline_edit')
    expect(context).toContain('hashline_edit')
  })

  it('prioritizes multiple god nodes by criticality then inDegree', () => {
